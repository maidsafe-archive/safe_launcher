  ...
  describe('Add key, remove key and toggle key from filter', () => {
    ...
    it('Should be able to Add and remove keys in filter and toggle filter', () => (
      adUtils.getSignKeyAt(authToken, adHandleId, 0)
        ...
        .then(() => adUtils.getMetadata(authToken, adHandleId))
        .should.be.fulfilled()
        .then(res => should(res.data.filterType.toLowerCase()).be.equal('whitelist'))
        .then(() => createSD())
        .then(handleId => (dataIdHandle = handleId))
        .then(() => adUtils.append(authToken, adHandleId, dataIdHandle))
        .should.be.rejectedWith(Error)
        ...
        .then(() => adUtils.getSignKey(authToken))
        .should.be.fulfilled()
        .then(res => adUtils.addToFilter(authToken, adHandleId, [res.data.handleId]))
        .should.be.fulfilled()
        .then(() => adUtils.post(authToken, adHandleId))
        .should.be.fulfilled()
        .then(() => dropAndGetHandle(AD_NAME))
        ...
        .then(() => adUtils.getMetadata(authToken, adHandleId))
        .should.be.fulfilled()
        .then(res => should(res.data.dataLength).be.equal(2))
        .then(() => adUtils.toggleFilter(authToken, adHandleId))
        .should.be.fulfilled()
        .then(() => adUtils.post(authToken, adHandleId))
        .should.be.fulfilled()
        ...
        .then(() => adUtils.getMetadata(authToken, adHandleId))
        .should.be.fulfilled()
        .then(res => should(res.data.filterType.toLowerCase()).be.equal('blacklist'))
        .then(() => adUtils.removeFromFilter(authToken, adHandleId, [signKeyHandleId]))
        .should.be.fulfilled()
        .then(() => createSD())
        .then(handleId => (dataIdHandle = handleId))

          ...
          D::Error: ::std::fmt::Debug
{
    ...
    if api_dest != "safe-api" {
        return Err(::errors::LauncherError::SpecificParseError(format!("Unrecognised token \
                                                                        \"{}\" in endpoint \
                                                                        path.",
                                                                       api_dest)));
    }
    ...
    if version_str.len() < 4 || version_str.remove(0) != 'v' {
        return Err(::errors::LauncherError::SpecificParseError("Unparsable version in endpoint \
                                                                path."
                                                                   .to_string()));
    }
          ...
          D::Error: ::std::fmt::Debug
{
    ...
    match &module[..] {
        ...
        _ => {
            Err(::errors::LauncherError::SpecificParseError(format!("Unrecognised module \"{}\" \
                                                                     in endpoint path.",
                                                                    module)))
        }
    }

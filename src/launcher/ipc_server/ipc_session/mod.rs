...
pub type EventSenderToSession<EventSubset> = EventSender<events::IpcSessionEventCategory,
                                                         EventSubset>;
...
impl IpcSession {
    pub fn new(server_event_sender: EventSenderToServer<IpcSessionEvent>,
        ...
        let ipc_session_joiner = thread!(IPC_SESSION_THREAD_NAME, move || {
            ...
            let joiner = authenticate_app::verify_launcher_nonce(ipc_stream,
        ...
        Ok((RaiiThreadJoiner::new(ipc_session_joiner),
                ...
                events::IpcSessionEventCategory::ExternalEvent => {
                    if let Ok(external_event) = self.external_event_rx.try_recv() {
                        match external_event {
                            events::ExternalEvent::AppDetailReceived(app_detail) => {
                                self.on_app_detail_received(app_detail)
                            }
                            events::ExternalEvent::ChangeSafeDriveAccess(is_allowed) => {
                                self.on_change_safe_drive_access(is_allowed)
                            }
    ...
    fn on_auth_data_received(&mut self, auth_data: events::event_data::AuthData) {
        ...
        let _ = send_one!((self.temp_id, auth_data.str_nonce),
    ...
    fn on_app_detail_received(&mut self, app_detail: Box<events::event_data::AppDetail>) {
        ...
        if let Some(mut ipc_stream) = self.get_ipc_stream_or_terminate() {
            match ecdh_key_exchange::perform_ecdh_exchange(&mut ipc_stream,
                                                           unwrap_option!(self.app_nonce,
                                                                          "Logic Error - \
                                                                           Report a bug."),
                                                           unwrap_option!(self.app_pub_key,
                                                                          "Logice Error - \
                                                                           Report a bug.")) {
                Ok((symm_nonce, symm_key)) => {
                    ...
                    self.raii_joiner =
                        secure_communication::SecureCommunication::new(app_detail.client,
                                                                       event_sender,
                                                                       symm_key,
                                                                       symm_nonce,
                                                                       ipc_stream,
...
mod tests {
    ...
    impl json::ToJson for HandshakePayload {
        fn to_json(&self) -> json::Json {
            ...
            assert!(tree.insert("launcher_string".to_string(),
                                self.launcher_string.to_json())
                        .is_none());
            assert!(tree.insert("asymm_nonce".to_string(), base64_nonce.to_json()).is_none());
            assert!(tree.insert("asymm_pub_key".to_string(),

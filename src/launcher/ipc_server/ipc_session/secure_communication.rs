...
impl SecureCommunication {
    pub fn new(client           : Arc<::std::sync::Mutex<Client>>,
               ...
               safe_drive_access: Arc<Mutex<bool>>) -> RaiiThreadJoiner {
            ...
            let safe_drive_dir_key = {
                ...
                eval_send_one!(user_root_dir_listing.find_sub_directory(
                    &::config::SAFE_DRIVE_DIR_NAME.to_string()).ok_or(
                        LauncherError::from("Could not find SAFEDrive")),
                    &observer)
                    .get_key()
                    .clone()
            };
    ...
    fn on_receive_payload(&self, cipher_text: &[u8]) -> parser::ResponseType {
        ...
        parser::begin_parse(self.parser_parameters.clone(),
                            &mut json::Decoder::new(json_request))
    }
    ...
    fn get_encrypted_normal_response(&self,
        ...
        let normal_response = LauncherNormalResponse {
            ...
            data: unwrap_result!(json::Json::from_str(&data[..])),
        };

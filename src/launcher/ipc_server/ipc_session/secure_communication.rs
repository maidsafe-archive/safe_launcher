...
pub struct SecureCommunication {
    observer         : ::launcher::ipc_server::ipc_session::EventSenderToSession<
                           ::launcher::ipc_server::ipc_session::events::SecureCommunicationEvent>,
...
impl SecureCommunication {
    pub fn new(client           : ::std::sync::Arc<::std::sync::Mutex<::safe_core::client::Client>>,
        ...
        let joiner = thread!(SECURE_COMM_THREAD_NAME, move || {
            let safe_drive_dir_key = {
                ...
                let user_root_dir_listing =
                    eval_send_one!(dir_helper.get_user_root_directory_listing(), &observer);
                eval_send_one!(user_root_dir_listing.find_sub_directory(
                    &::config::SAFE_DRIVE_DIR_NAME.to_string()).ok_or(
                        ::errors::LauncherError::from("Could not find SAFEDrive")),
                    &observer)
                    .get_key()
                    .clone()
            };
            ...
            match self.on_receive_payload(&cipher_text) {
                Ok(parser_response) => {
                    if let Some(response_json_str) = parser_response {
                        match self.get_encrypted_normal_response(&cipher_text, response_json_str) {
                            Ok(response_cipher) => {
                                eval_send_one!(self.ipc_stream.write(response_cipher),
                                               &self.observer)
                            }
                            Err(err) => {
                                debug!("{:?} - Failed to construct a normal response for peer.",
                                       err)
                            }
                        }
                    } else {
                        match self.get_encrypted_error_response(&cipher_text, None) {
                            ...
                            Err(err) => {
                                debug!("{:?} - Failed to construct a response error for peer.", err)
                            }
                        }
                    }
                }
                Err(err) => {
                    match self.get_encrypted_error_response(&cipher_text, Some(err)) {
                        Ok(response_cipher) => {
                            eval_send_one!(self.ipc_stream.write(response_cipher), &self.observer)
                        }
                        Err(err) => {
                            debug!("{:?} - Failed to construct a response error for peer.", err)
                        }
    ...
    fn on_receive_payload(&self, cipher_text: &[u8]) -> ::launcher::parser::ResponseType {
        let plain_text = try!(::sodiumoxide::crypto::secretbox::open(&cipher_text,
                                                                     &self.symm_nonce,
                                                                     &self.symm_key)
                                  .map_err(|()| {
                                      ::errors::LauncherError::SymmetricDecipherFailure
                                  }));
        ...
        ::launcher::parser::begin_parse(self.parser_parameters.clone(),
    ...
    fn get_encrypted_normal_response(&self,
                                     orig_payload: &[u8],
                                     data: String)
                                     -> Result<Vec<u8>, ::errors::LauncherError> {
        ...
        let cipher_text = ::sodiumoxide::crypto::secretbox::seal(&json_str.into_bytes(),
                                                                 &self.symm_nonce,
                                                                 &self.symm_key);
    ...
    fn get_encrypted_error_response(&self,
                                    orig_payload: &[u8],
                                    error: Option<::errors::LauncherError>)
                                    -> Result<Vec<u8>, ::errors::LauncherError> {

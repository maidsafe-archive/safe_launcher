...
pub struct Launcher {
    ...
    app_handler_event_sender: mpsc::Sender<AppHandlerEvent>,
}
...
impl Launcher {
    ...
    pub fn new(client: Client) -> Result<Launcher, LauncherError> {
        let client = Arc::new(::std::sync::Mutex::new(client));

        let directory_helper = directory_helper::DirectoryHelper::new(client.clone());

        let launcher_config_dir_name = LAUNCHER_GLOBAL_DIRECTORY_NAME.to_string();
    ...
    pub fn get_app_handler_event_sender(&self) -> &mpsc::Sender<AppHandlerEvent> {
...
mod tests {
    ...
    fn initialise_safe_drive_dir() {
        ...
        let client = unwrap_result!(Client::create_account(keyword.clone(),
                                                           pin.clone(),
                                                           password.clone()));

        let safe_drive_directory_name = SAFE_DRIVE_DIR_NAME.to_string();
        let arc_client = Arc::new(Mutex::new(client));
        let directory_helper = DirectoryHelper::new(arc_client.clone());

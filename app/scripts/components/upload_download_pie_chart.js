...
var UploadDownloadPieChart = React.createClass({
  ...
  bytesToSize: function(bytes) {
    ...
    var i = parseInt(MathObj.floor(MathObj.log(bytes) / MathObj.log(1024)));
    if (i === 0) {
      return bytes + ' ' + sizes[i];
    }
    return (bytes / MathObj.pow(1024, i)).toFixed(1) + ' ' + sizes[i];
  },
  render: function() {
    ...
    return React.DOM.span({ key: 'pieChart', className: 'pie-chart-b' }, [
      React.DOM.div({ key: 'pieChartLegends', className: 'legends' }, [
        React.DOM.div({ key: 'pieChartLegendsDwn', className: 'legends-i download' }, [
          ...
          React.DOM.div({ key: 'pieChartLegendsDwnVal', className: 'legends-i-val' },
          (this.dataAvailable ? this.bytesToSize(this.props.download) : 'No data yet.'))
        ]),
        React.DOM.div({ key: 'pieChartLegendsUpl', className: 'legends-i upload' }, [
          ...
          React.DOM.div({ key: 'pieChartLegendsUplVal', className: 'legends-i-val' },
          (this.dataAvailable ? this.bytesToSize(this.props.upload) : 'No data yet.'))
        ])
      ])
    ]);
  },
  initD3: function() {
    ...
    var height = 100;
    var padding = 10;
    var radius = MathObj.min(width, height) / 2;

...
let deleteOrGetDirectory = function (req, res, isDelete, next) {
  ...
  if (!sessionInfo) {
    log.error(`NFS - ${req.id} :: ${isDelete ? 'DELETE' : 'GET'} directory :: Unauthorised request`);
  ...
  const exec = async() => {
    ...
    try {
      ...
      log.warn(`NFS - ${req.id} :: ${isDelete ? 'DELETE' : 'GET'} directory :: Caught exception - ${parseExpectionMsg(e)}`);
      responseHandler(e);
    }
  };
  exec();
  ...
  const exec = async() => {
    ...
    try {
      if (isFile) {
        ...
        log.debug(`NFS - ${req.id} :: File ${action === FILE_OR_DIR_ACTION.move ? 'moved' : 'copied'}`);
      } else {
        ...
        log.debug(`NFS - ${req.id} :: Directory ${action === FILE_OR_DIR_ACTION.move ? 'moved' : 'copied'}`);
      }
      responseHandler();
    } catch (e) {
      log.warn(`NFS - ${req.id} :: MOVE ${isFile ? 'file' : 'directory'} :: Caught exception - ${parseExpectionMsg(e)}`);
      responseHandler(e);
    }
  };
  exec();
...
export var createDirectory = function (req, res, next) {
  ...
  nfs.createDirectory(app, dirPath, reqBody.metadata, reqBody.isPrivate, false, rootPath)
    .then(responseHandler, responseHandler, (e) => log.error(`NFS - ${req.id} :: Create Directory :: ${e}`));
};
